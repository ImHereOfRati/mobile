...
export function GeofenceFormScreen({ id }: { id?: number }) {
  ...
  async function submit() {
    if (draft === null) return;
    const resolved = { ...draft, message: resolveDraftMessage(draft) };
    const nextErrors = validateGeofenceDraft(resolved);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length > 0) return;
    setSaving(true);
    try {
      await bridge.registerGeofence(toBridgeInput(resolved, recipients));
  ...
  const resolvedMessage = resolveDraftMessage(draft);
      ...
      >
        ...
        <section className="feature-form__section">
          ...
          <TextField
            ...
            helperText={
              hasDeviceRecipients
                ? "기기 연락처(SMS) 수신자가 있어 장소 이름 기준 문구가 자동으로 적용돼요."
