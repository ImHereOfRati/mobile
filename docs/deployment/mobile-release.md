# Mobile release guide (Android)

This document describes the Android mobile release procedure. iOS is not
currently planned for release.
...
iOS는 현재 제공 계획이 없습니다. 아래 내용은 향후 재검토 시 참고할 수
있는 보류된 설정 메모이며, iOS 빌드나 스토어 배포에는 사용하지 않습니다.
...
- `ios/Runner/Runner.entitlements` with `aps-environment` (Xcode rewrites it
  to `production` when exporting for the App Store) and
  `com.apple.developer.applesignin`. Both capabilities must also be enabled on
  the App ID in the developer portal, or signing fails.
